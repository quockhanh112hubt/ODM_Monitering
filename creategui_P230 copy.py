...
class ProductionStatusBoard(QWidget):
    ...
    def create_ui(self):
        ...
        # Tạo hình ảnh màu ở góc trên bên phải (ngoài header)
        self.color_image_label = QLabel(self)
        self.color_image_label.setFixedSize(83, 340)  # Kích thước để vừa với chiều cao header
        self.color_image_label.setStyleSheet("background-color: transparent; border: none;")  # Bỏ viền và nền
        self.color_image_label.setAlignment(Qt.AlignCenter)
        self.color_image_label.setScaledContents(False)  # Tắt auto scale để kiểm soát tốt hơn
        self.color_image_label.show()  # Đảm bảo widget được hiển thị
        # Vị trí sẽ được cập nhật trong resizeEvent
    ...
    def update_color_image(self, color_value):
        ...
        if os.path.exists(image_path):
            ...
            label_size = self.color_image_label.size()
            # Scale ảnh với chất lượng cao, giữ tỉ lệ và vừa với label
            ...
            self.color_image_label.setPixmap(scaled_pixmap)
        else:
    ...
    def resizeEvent(self, event):
        """Cập nhật vị trí hình ảnh khi thay đổi kích thước cửa sổ"""
        super().resizeEvent(event)
        if hasattr(self, 'color_image_label'):
            # Đặt hình ảnh ở góc trên bên phải, ngang với header
            window_width = self.width()
            self.color_image_label.move(window_width - 200, 10)  # Dịch sang trái 20px (từ 170 thành 190)
            self.color_image_label.raise_()  # Đảm bảo luôn ở trên cùng
