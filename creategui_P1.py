...
class ProductionStatusBoard(QWidget):
    def __init__(self, menu_bar):
        ...
        self.setWindowTitle("PRODUCTION STATUS BOARD")
    ...
    def create_ui(self):
        ...
        header_label = QLabel("P1 PRODUCTION STATUS BOARD")
        ...
        self.main_line_label = QLabel("P1EZ SUB LINE")
    ...
    def update_data_from_db(self):
        ...
        if self.conn:
            ...
            try:
                ...
                if selected_line == "P1EZ MAIN1 LINE":
                    query = f"""
                        select count(*) as prod
                        from ITMV_KTNG_DB.dbo.AFA_CHARGE_INSPECTION_HISTORY a
                        where row_id in (
                            select max(row_id)
                            from ITMV_KTNG_DB.dbo.AFA_CHARGE_INSPECTION_HISTORY b
                    ...
                    """
                elif selected_line == "P1EZ SUB LINE":
                    query = f""" 
                        select count(*) as prod
                        from ITMV_KTNG_DB.dbo.AFA_RESISTANCE_TEST_HISTORY a
                        where row_id in (
                            select max(row_id)
                            from ITMV_KTNG_DB.dbo.AFA_RESISTANCE_TEST_HISTORY b
                    ...
                    """
                elif selected_line == "P1EZ MAIN2 LINE":
                    query = f""" 
                        select count(*) as prod
                        from ITMV_KTNG_DB.dbo.AFA_VERIFICATION_HISTORY a
                        where row_id in (
                            select max(row_id)
                            from ITMV_KTNG_DB.dbo.AFA_VERIFICATION_HISTORY b
                            where b.mcu_id = a.mcu_id
                            and total_judgment = 'PASS'
                            and work_time >= '{start_work_time_str}'
                            and work_time < '{end_work_time_str}'
                    ...
                    """
                elif selected_line == "P1EZ FINAL LINE":
                    query = f"""
                        select count(*) as prod
                        from ITMV_KTNG_DB.dbo.AFA_FINAL_TEST_HISTORY a
                        where row_id in (
                            select max(row_id)
                            from ITMV_KTNG_DB.dbo.AFA_FINAL_TEST_HISTORY b
                ...
                if query:
                    ...
                    if selected_line == "P1EZ PACKING LINE":
                        query = None
