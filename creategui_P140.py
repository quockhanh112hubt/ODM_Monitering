...
class ProductionStatusBoard(QWidget):
    ...
    def create_ui(self):
        ...
        header_label = QLabel("P1 4.0 PRODUCTION STATUS BOARD")
            ...
            try:
                ...
                if selected_line == "SUB LINE : Cover Heater Assy 기능 검사":
                    query = f"""
                        ...
                        where row_id in (
                            select max(row_id)
                            from ITMV_KTNG_DB.dbo.AFA_P140_COVER_HEATER_FUNCTION_HISTORY b
                            where b.COVER_HEATER_ID = a.COVER_HEATER_ID
                    ...
                    query = f""" 
                        ...
                        where row_id in (
                            select max(row_id)
                            from ITMV_KTNG_DB.dbo.AFA_P140_CHARGE_CURRENT_HISTORY b
                    ...
                    query = f"""
                        ...
                        where row_id in (
                            select max(row_id)
                            from ITMV_KTNG_DB.dbo.AFA_P140_FINAL_TEST_HISTORY b
                            where b.mcu_id = a.mcu_id
                            and total_judgment = 'PASS'
                            and work_time >= '{start_work_time_str}'
                            and work_time < '{end_work_time_str}'
